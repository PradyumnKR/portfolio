...
export default function Contact() {
  ...
  // 5. Replace the three placeholder strings below in the emailjs.sendForm call
  ...
  const getButtonStyles = () => {
    if (status === 'success') return 'border-[rgba(74,222,128,0.5)] text-[#4ade80]';
    if (status === 'error') return 'border-[rgba(239,68,68,0.5)] text-[#ef4444]';
      ...
      <div className="max-w-[1100px] mx-auto px-8 w-full">
            ...
            <div className="relative z-10 text-left flex flex-col h-full">
              ...
              <div className="space-y-0 border-l-2 border-[rgba(200,169,110,0.15)] pl-4">
                <div className="py-4 border-b border-[rgba(200,169,110,0.1)]">
                  <div className="flex items-center mb-1">
                    ...
                    <p className="text-[#c8a96e] opacity-55 text-[9px] uppercase tracking-[0.2em] font-mono">Office Hours</p>
                  </div>
                  <p className="text-[rgba(240,234,216,0.8)] font-mono text-[12px]">09:00 — 21:00 IST</p>
                </div>
                <div className="py-4 border-b border-[rgba(200,169,110,0.1)]">
                  <div className="flex items-center mb-1">
                    <span className="text-[#c8a96e] opacity-50 mr-[6px] text-[10px]">›</span>
                    <p className="text-[#c8a96e] opacity-55 text-[9px] uppercase tracking-[0.2em] font-mono">Station Location</p>
                  </div>
                  <p className="text-[rgba(240,234,216,0.8)] font-mono text-[12px]">Jaipur, Rajasthan // India</p>
                </div>
                <div className="py-4">
                  <div className="flex items-center mb-1">
                    <span className="text-[#c8a96e] opacity-50 mr-[6px] text-[10px]">›</span>
              ...
              <div className="mt-auto pt-4 border-t border-[rgba(200,169,110,0.1)]">
                <p className="text-[#c8a96e] opacity-40 text-[12px] italic font-serif leading-relaxed">
                  "Every word transmitted is a legacy recorded in the archives of time."
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="mt-6 w-full">
          <div className="border border-[rgba(200,169,110,0.1)] border-t-[rgba(200,169,110,0.25)] p-4 px-6 flex flex-col md:flex-row justify-between items-stretch md:items-center bg-transparent mt-6">
            {[
              { label: 'Encryption', val: 'Active', pulse: 'status-blink-online' },
              { label: 'Archive', val: 'Online', pulse: 'status-blink-online' },
              { label: 'Response', val: '24—48 HRS', pulse: 'status-blink-standby' }
            ].map((item, i) => (
              <div key={i} className="flex flex-1 items-center justify-center relative">
                <div className="flex items-center gap-3 py-3 md:py-0 w-full md:w-auto justify-center">
                  <div className={`w-[5px] h-[5px] rounded-full ${item.pulse}`}></div>
                  <div className="font-mono text-[10px] tracking-[0.15em] uppercase whitespace-nowrap">
                    <span className="text-[#c8a96e] opacity-50">{item.label}:</span>
                    <span className="text-[#c8a96e] opacity-85 ml-1">{item.val}</span>
                  </div>
                </div>
                {i < 2 && (
                  <>
